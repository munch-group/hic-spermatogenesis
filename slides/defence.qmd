...
1. Let $c(x)$ quantify the compartment assignment for bin $x$: find $c(x)$ for all $x$
2. Interaction score between loci $x$ and $y$ is $I(x,y) = c(x)c(y)$, with $I(x,y) > 0$ when they share the same assignment
3. Minimize the mean squared error (MSE) between observed interaction frequencies and $c(x)c(y)$
4. PCA finds the first principal component that minimizes MSE, assigning loci to compartment A ($c(x) > 0$) or B ($c(x) < 0$)
5. We can restrict the genomic range (viewframe) in which to minimize MSE (e.g. chromosome arms)
:::

:::notes
1. "To begin, we define $c(x)$, which represents the compartment assignment for each bin $x$. The goal is to determine $c(x)$ for all the bins in the dataset. This step is essential because $c(x)$ will later guide us in categorizing the loci into their respective compartments."
2. "Next, we define the interaction score $I(x, y)$ between loci $x$ and $y$. This score is simply the product of their respective compartment assignments, $c(x)c(y)$. The important thing to note is that $I(x, y)$ is positive if the loci share the same compartment assignment. This means they are likely in the same functional region of the genome, which is critical for the analysis."
3. "We then move on to minimizing the **mean squared error**, or MSE. The MSE quantifies how well the interaction frequencies observed in the data match the predicted interaction scores, $c(x)c(y)$. Minimizing this error helps ensure that our model is accurately capturing the observed patterns of interaction between the loci."
4. "Finally, we apply Principal Component Analysis, or PCA, to find the first principal component that minimizes the MSE. This allows us to categorize the loci into two compartments: **Compartment A**, where $c(x) > 0$, and **Compartment B**, where $c(x) < 0$. This step is crucial because it provides a way to simplify and visualize the compartmentalization of the genome."
