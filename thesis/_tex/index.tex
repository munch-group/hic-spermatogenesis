...
parsing runs, see Figure~\ref{fig-pairtools-qc}. The peak at around 250
bp is a techical artefact and should not be parsed into valid contacts.
...
\item[$\boxtimes$]
  ...
  \item[$\boxtimes$]
    discuss the choice of viewframe
  \item[$\square$]
...
of the centromere. We confidently rationalize that the reason we have to
...
the two runs end up with the same distribution of cis pairs.
...
but only is for visualization, it looses transparency when hiding empty
...
shortest segment is just below 100kb. Thus, the 100kb resolution is
deemed sufficient to capture compartments that are within the range of
the regions under selection.

\section{Speculations}\label{speculations}

\subsection{Gene Drive and
Selfishness}\label{gene-drive-and-selfishness}
