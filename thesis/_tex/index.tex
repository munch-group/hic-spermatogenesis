...
\hypersetup{
  ...
  pdfkeywords={\xmpquote{Hi-C}, \xmpquote{Chromatin
Compartments}, \xmpquote{Selection on X}, \xmpquote{Selfish gene
drive}},
  hidelinks,
...
gene tree incongruent with the species tree (Mailund et al. 2014).
Briefly, it complicates phylogenetic inference as it implies that
divergence time does represent speciation time. But, by solving the
incongruency with a maximum likelihood approach (see Mailund et al.
2014), the relation between the ILS proportion, \(p\), time between two
speciation events, \(\Delta\tau\), and effective population size,
\(N_e\), is given by the formula
...
\pandocbounded{\includegraphics[keepaspectratio]{illustrations/chromatin-intro.jpg}}

}

\caption{\label{fig-chromatin-architecture-overview}DNA is organized at
multiple hierarchical structures. Figure from Misteli (2020).}
...
type of reads (Lajoie et al. 2015). Thus, we need specialized tools for
rescuing chimeric reads. That said, we have to be cautious
distinguishing the intended chimerism for Hi-C and that of technical
artefacts. Any software for local alignment can be used for aligning
reads from a Hi-C library. However, one should make sure to disable
paired-end rescue mode if possible, otherwise each read in a pair (each
mate) should be aligned separately (Lajoie et al. 2015). This removes
the assumption that the distance between mates fits a known distribution
because the genomic sequences originate from a continuous DNA-fragment.
For example, the \emph{bwa-mem} (Li 2013) implementation of this (the
...
uninformative (Lajoie et al. 2015). Usually, \emph{weird} events are
demeed uninformative as well, as it is challenging to distinguish a
sequencing error from the result of a diploid fragment. PCR duplicates
should be discarded as well, having either identical genomic sequence,
or sharing exact 5' alignment positions of the pair (Lajoie et al. 2015;
Bicciato and Ferrari 2022, Ch. 1). The probability that such pairs are
valid (i.e.~there are multiple of the same pairs) is very low. We also
have to distinguish between molecules with only a single ligation event
(one-way contact) or multiple ligation events (multi-way contacts). For
that, a descision should be made on whether to 1) discard molecules with
multiple ligations, 2) report one of the ligations (e.g.~the 5'-most in
both directions), or 3) report all events on a molecule.
...
matrix without further modifications (Lajoie et al. 2015). Briefly, each
genomic bin is assigned a value reflecting its compartment identity,
where positive values indicate compartment A and negative values
indicate compartment B. The interaction score between two loci is
...
initially defined as sub-Mb chromatin structures (Lajoie et al. 2015),
but currently the definition seems to vary based on the method of
...
aligners used for regular paired-end sequencing libraries (Lajoie et al.
(2015)). HiCExplorer provide examples for both \emph{bwa} and
\emph{bowtie2}, so I used both with recommended settings. In both cases,
the aligner outputs a .bam-file for each mate (\texttt{sample\_R1.bam}
and \texttt{sample\_R2.bam}), and HiCExplorer performs the parsing,
deduplication, and filtering of the reads and builds the raw interaction
matrix in a single command,
...
\emph{trans} contacts (Bicciato and Ferrari 2022, 236; Lieberman-Aiden
et al. 2009), and HiCExplorer states it is usually below 10\% for a
high-quality library. The high fraction may be mitigated by enforcing a
stricter \emph{mapq} threshold for a valid Hi-C pair, as we also observe
higher-than expected valid contacts. However, we continue with the
current matrices.
...
coverage throughout(Lajoie et al. 2015), as restriction enzymes do not
bind equally to all regions of the genome, and therefore, some bins will
be underrepresented as an artefact of binding/cutting efficieny of the
...
segments, we use \emph{segment\_liftover} (Gao et al. 2018), resulting
in much more similar segments to the original
...
are unclear. The latest methodological review is from 2015 (Lajoie et
al. 2015), and many things have progressed since then---a previously
mentioned example being the update to \emph{bwa} so mapping reads
seperately is no longer necessary. Additionally, a Springer Protocol
about Hi-C Data analysis (Bicciato and Ferrari 2022) is more recently
published, but I find that it more so establishes a range of
...
in Lajoie et al. (2015). The contact matrices generated from HiCExplorer
improved when correcting, but still, the visible pattern closely
delimited by low-count bins. This undesirable feature may be mitigated
by passing a stricter filtering threshold to \texttt{hicCorrectMatrix}.
...
litterature (Lajoie et al. 2015), recommending partitioning the
chromosome into its arms \emph{if the first eigenvector only captures
the arms}.
...
(Bravo Núñez et al. 2018); \emph{class one drivers} affect chromosome
segregation in meiosis, and \emph{killer meiotic drivers} will sabotage
meiotic product that have not inherited the driving allele (Bravo Núñez
et al. 2018). This happens regardless of the fitness effects of the
developed organism, and is there often a factor offsetting classical
selection. Even though the implications of such systems are potentially
detrimental, they are notoriously difficult to detect. A circumstance
contributing to the difficulty is that most genetic experiments are done
in homozygotes. Bravo Núñez et al. (2018) state that the general choice
...
(Bravo Núñez et al. 2018). Over time, as there is neither selfish nor
selective pressure to maintain the driving mechanism of a fixed driver,
the mechanism will decay as it accumulates inactivating mutations. As a
result, genetic drivers are said to be transient (Bravo Núñez et al.
2018) on an evolutionary timescale, unless it is linked to another
positively selected allele (Jaenike 2001). Interestingly, gene drive is
much more well-documented on sex chromosomes than for autosomes.
Possibly because the sex chromosome meiotic drive inherently causes a
skewed sex ratio, more notably raising a flag for further analysis. Or,
because X and Y do not recombine, allowing for killer-supressor gene
pairs to stay on separate chromosomes. The consequences of sex
...
ratio selection} (Lindholm et al. 2016.), as the individuals that
produce offspring of the minor-sex have higher fitness.
...
\emph{Bioinformatics} 36 (1): 311--16.
...
Christophe Dervieux. 2024. \emph{{Quarto}}. V. 1.4. Released February.
\url{https://doi.org/10.5281/zenodo.5960048}.

\bibitem[\citeproctext]{ref-anaconda}
Anaconda Software Distribution. 2016. \emph{Anaconda Software
...
Bersaglieri, Todd, Pardis C. Sabeti, Nick Patterson, et al. 2004.
{``Genetic {Signatures} of {Strong Recent Positive Selection} at the
{Lactase Gene}.''} \emph{The American Journal of Human Genetics} 74 (6):
1111--20. \url{https://doi.org/10.1086/421051}.
...
Biology}. Springer US. \url{https://doi.org/10.1007/978-1-0716-1390-0}.
...
DevTeam, SRA Toolkit. 2024. {``{SRA Toolkit}.''} Wiki. In \emph{GitHub}.
...
Dixon, Jesse R., Inkyung Jung, Siddarth Selvaraj, et al. 2015.
{``Chromatin Architecture Reorganization During Stem Cell
Differentiation.''} \emph{Nature} 518 (7539): 331--36.
...
Divergence}.''} \emph{PLOS Genetics} 11 (8): e1005451.
...
GenomeDK. 2023. \emph{Gwf Workflow Manager}.
\href{https://github.com/gwforg/gwf.git}{Https://github.com/gwforg/gwf.git}.

\bibitem[\citeproctext]{ref-github}
GitHub, Inc. n.d. \emph{GitHub}.
...
Imakaev, Maxim, Geoffrey Fudenberg, Rachel Patton McCord, et al. 2012.
{``Iterative Correction of {Hi-C} Data Reveals Hallmarks of Chromosome
Organization.''} \emph{Nature Methods} 9 (10): 999--1003.
\url{https://doi.org/10.1038/nmeth.2148}.

\bibitem[\citeproctext]{ref-auityourguidegai}
IT, AU. n.d. \emph{Your Guide to {GAI} Responsibly}.
Https://medarbejdere.au.dk/en/gai.
...
and Birgit Scmidt. IOS Press. \url{https://eprints.soton.ac.uk/403913/}.
...
Li, Heng. 2013. \emph{Aligning Sequence Reads, Clone Sequences and
Assembly Contigs with BWA-MEM}. \url{https://arxiv.org/abs/1303.3997}.

\bibitem[\citeproctext]{ref-lieberman-aiden2009comprehensivemappinglongrange}
Lieberman-Aiden, Erez, Nynke L. Van Berkum, Louise Williams, et al.
2009. {``Comprehensive {Mapping} of {Long-Range Interactions Reveals
Folding Principles} of the {Human Genome}.''} \emph{Science} 326 (5950):
289--93. \url{https://doi.org/10.1126/science.1181369}.

\bibitem[\citeproctext]{ref-lindholm2016ecologyevolutionarydynamics}
Lindholm, Anna K., Kelly A. Dyer, Renée C. Firman, et al. 2016. {``The
...
\bibitem[\citeproctext]{ref-misteli2020selforganizinggenomeprinciples}
...
Munch, Kasper. 2024. \emph{Munch-Group}.
Https://munch-group.org/research.html.

\bibitem[\citeproctext]{ref-openchromosomecollective}
{``{Open2C}.''} n.d. Resource. In \emph{Open {Chromosome Collective}}.
Https://open2c.github.io/.

\bibitem[\citeproctext]{ref-open2c2022cooltoolsenablinghighresolution}
Open2C, Nezar Abdennur, Sameer Abraham, et al. 2022. \emph{Cooltools:
Enabling High-Resolution {Hi-C} Analysis in {Python}}. Bioinformatics.
\url{https://doi.org/10.1101/2022.10.31.514564}.

\bibitem[\citeproctext]{ref-open2c2024pairtoolssequencingdata}
Open2C, Nezar Abdennur, Geoffrey Fudenberg, et al. 2024. {``Pairtools:
{From} Sequencing Data to Chromosome Contacts.''} \emph{PLOS
Computational Biology} 20 (5): e1012164.
...
Technical Publishing System.''} In \emph{Posit}.

\bibitem[\citeproctext]{ref-ramirez2018highresolutiontadsreveal}
Ramírez, Fidel, Vivek Bhardwaj, Laura Arrigoni, et al. 2018.
{``High-Resolution {TADs} Reveal {DNA} Sequences Underlying Genome
Organization in Flies.''} \emph{Nature Communications} 9 (1): 189.
\url{https://doi.org/10.1038/s41467-017-02525-w}.

\bibitem[\citeproctext]{ref-sayers2022databaseresourcesnational}
Sayers, Eric W, Evan E Bolton, J Rodney Brister, et al. 2022.
{``Database Resources of the National Center for Biotechnology
Information.''} \emph{Nucleic Acids Research} 50 (D1): D20--26.
\url{https://doi.org/10.1093/nar/gkab1112}.

\bibitem[\citeproctext]{ref-skov2023extraordinaryselectionhuman}
Skov, Laurits, Moisès Coll Macià, Elise Anne Lucotte, et al. 2023.
{``Extraordinary Selection on the Human {X} Chromosome Associated with
Archaic Admixture.''} \emph{Cell Genomics} 3 (3): 100274.
\url{https://doi.org/10.1016/j.xgen.2023.100274}.

\bibitem[\citeproctext]{ref-sorensen2023genomewidecoancestryreveals}
Sørensen, Erik F., R. Alan Harris, Liye Zhang, et al. 2023.
...
Team, The Matplotlib Development. 2024. \emph{Matplotlib:
{Visualization} with {Python}}. Zenodo.
\url{https://doi.org/10.5281/ZENODO.10916799}.

\bibitem[\citeproctext]{ref-thegalaxycommunity2024galaxyplatformaccessible}
The Galaxy Community, Linelle Ann L Abueg, Enis Afgan, et al. 2024.
{``The {Galaxy} Platform for Accessible, Reproducible, and Collaborative
Data Analyses: 2024 Update.''} \emph{Nucleic Acids Research} 52 (W1):
W83--94. \url{https://doi.org/10.1093/nar/gkae410}.
...
Wang, Yao, Hanben Wang, Yu Zhang, et al. 2019. {``Reprogramming of
{Meiotic Chromatin Architecture} During {Spermatogenesis}.''}
\emph{Molecular Cell} 73 (3): 547--561.e6.
\url{https://doi.org/10.1016/j.molcel.2018.11.019}.

\bibitem[\citeproctext]{ref-warren2020sequencediversityanalyses}
Warren, Wesley C., R. Alan Harris, Marina Haukness, et al. 2020.
...
Wolff, Joachim, Vivek Bhardwaj, Stephan Nothjunge, et al. 2018.
{``Galaxy {HiCExplorer}: A Web Server for Reproducible {Hi-C} Data
Analysis, Quality Control and Visualization.''} \emph{Nucleic Acids
Research} 46 (W1): W11--16. \url{https://doi.org/10.1093/nar/gky504}.

\bibitem[\citeproctext]{ref-wolff2020galaxyhicexplorer3}
Wolff, Joachim, Leily Rabbani, Ralf Gilsbach, et al. 2020. {``Galaxy
{HiCExplorer} 3: A Web Server for Reproducible {Hi-C}, Capture {Hi-C}
and Single-Cell {Hi-C} Data Analysis, Quality Control and
Visualization.''} \emph{Nucleic Acids Research} 48 (W1): W177--84.
\url{https://doi.org/10.1093/nar/gkaa220}.

\bibitem[\citeproctext]{ref-zuo2021stageresolvedhicanalyses}
Zuo, Wu, Guangming Chen, Zhimei Gao, et al. 2021. {``Stage-Resolved
{Hi-C} Analyses Reveal Meiotic Chromosome Organizational Features
Influencing Homolog Alignment.''} \emph{Nature Communications} 12 (1):
5827. \url{https://doi.org/10.1038/s41467-021-26033-0}.
