...
We redo Hi-C analyses from a 2019 paper on the latest reference genome
(\emph{rheMac10} or \emph{Mmul\_10}). We compare two Hi-C analysis
frameworks, \emph{HiCExplorer} and \emph{cooler/cooltools} (Open2C), on
a subset, finding Open2C to be most flexible and intuitive. The ICE
method (Iterative Correction and Eigendecomposition) was used to infer
...
Chromatin architecture, particularly the organization into A/B
compartments, may provide insights into the functional basis of these
patterns.

Here, we analyze the 3D chromatin structure of the X chromosome in
rhesus macaque using Hi-C data on the latest reference genome
(rheMac10). Our analysis extensively combines software tools that
emphasize reproducibility, ensuring that our analyses are transparent,
...
et al. 2019), and it is the basis of male reproduction. The specialized
...
distribution. Deeply understanding the molecular steps of reproduction
and how our genetic material is inherited is essential in biology,
bringing insight to areas such as speciation, population diversity, and
infertility.
...
chromosome. The Y chromosome exist only in males, and no more than one
copy is in each individual. The complexity increases when we tend to the
X chromosome, which exist as single-copy in males and as two copies in
females. This skewed ratio means that X chromosomes exist \(2/3\) of the
time in females and only \(1/3\) of the time in males, and it is more
exposed in males as there is no other copy to take over loss of
function. It is also the underlying reasoning for Haldane's rule
...
investigation, and several studies infer strong selection on the X
chromosomes across primates, as discussed in the following.
...
Additionally, sex-specific selective pressures, such as those linked to
the unique transmission and evolutionary dynamics of the X chromosome,
could contribute to this pattern. Further investigation is needed to
...
these regions may be under strong selective pressures or influenced by
structural genomic features that either limit recombination. Such
patterns suggest that certain chromosomal regions are either preserved
due to adaptive advantages or subjected to sex-biased gene flow, further
reinforcing the impact of male-driven admixture on shaping genomic
variation. These findings highlight the complexity of admixture dynamics
and their potential to create localized genomic signatures that deviate
from overall ancestry proportions.
...
they correlate with regions under selection across primates, we have yet
another mystery to explain.

Wang et al. (2019) found that through the stages of spermatogeneis,
...
spermatogenesis could make obvious starting points for investigating the
relationship between yet another set of megabase-spanning regions and
those of strong selection identified above.
...
\caption{\label{lst-reproduce}These three lines of code should ideally
be enough to reproduce the whole project.}
...
utilized. Several exist, but the most well-known must be
\emph{Snakemake}. However, we use the pragmatic (their own words),
lightweight workflow manager \emph{gwf}, which is optimized for the
GenomeDK insfrastructure, and has the benefit of in-house support.
...
was made into a custom \emph{gwf} workflow, a workflow manager developed
at GDK. I would like to thank GDK and Aarhus University for providing
...
reports are merged and visualized with \texttt{MultiQC} (Ewels et al.
2016) for each cell type.
...
\caption{\label{tbl-svedig-tabel-significant}Significant p-values across
...
\bottomrule\noalign{}
\endlastfoot
0 & 10Mb & ECH90 & jaccard & 0.02146 & 0.00611 & -\/-\/- & -\/-\/- &
0.0475 \\
1 & 10Mb & olive & proximity & -\/-\/- & -\/-\/- & -\/-\/- & -\/-\/- &
0.01614 \\
2 & 10Mb & olivehama & proximity & -\/-\/- & 0.01879 & 0.0123 & -\/-\/-
& 0.00128 \\
3 & arms & ECH90 & jaccard & 0.01656 & 0.00612 & 0.00534 & 0.02769 &
-\/-\/- \\
4 & arms & olivehama & proximity & -\/-\/- & -\/-\/- & 0.03564 & -\/-\/-
& -\/-\/- \\
...
comparing genomic intervals. In stead, this became a study more of Hi-C
...
tables generated in Python (see
Table~\ref{tbl-svedig-tabel-significant}) to translate into LaTeX (which
again is an idealistic typesetting system).
...
would have to modified to stop saving the plots as files in stead of
...
Dekker, and Kaplan 2015), and many things have progressed since then.
Additionally, a Springer Protocol about Hi-C Data analysis (Bicciato and
Ferrari 2022) is more recently published, but I find that it more so
establishes a range of possibilities than a practical guideline.

\section{On Results}\label{on-results}

The differences in the results the two tools produced were crucial, as I
could not reproduce the eigenvectors reported in Wang et al. (2019) with
HiCExplorer, as HiCExplorer has no (apparent) way of restricting the
viewframe of the eigendecomposition. The eigenvectors resulting from
Open2C line up very well with the ones from Wang et al. (2019). Here,
some of the maps were lined up and overlayed manually in a sketching
software. The predicted compartments from cooler matched very well with
the ones predicted by Wang et al. (2019) if their track was split in the
region of the centromere. We confidently rationalize that the reason we
have to split in the centromeric region is the newly, more accurate
reference genome. As it \emph{rheMac10} has closed a high amount of gaps
since last reference (most of which are expected around the centromere),
we expect more sequences to align in that region, resulting in a longer
genome as well.
