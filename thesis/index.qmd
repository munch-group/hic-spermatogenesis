...
As described, the pairtools module in MultiQC [@ewels2016multiqcsummarizeanalysis] was used to visualize results from `pairtools stats` for the two parsing runs, see @fig-pairtools-qc. The peak at around 250 bp is a techical artefact and should not be parsed into valid contacts. 
...
- [x] discuss the methods
  ...
  - [x] Matrices
  - [x] did not manage to implement their sliding window PCA
  - [x] discuss the choice of viewframe
...
Similarly, the choice of resolution should be made cautiously. Here, the resolution of the matrix is the same resolution as the resolution of the eigenvector. Therefore, a resolution can be chosen to reflect the genomic scale that we want to investigate, and the minimum resolution required to test if the compartments overlap with regions under selection. Although some of these regions are exceptionally large, the shortest segment is just below 100kb. Thus, the 100kb resolution is deemed sufficient to capture compartments that are within the range of the regions under selection. 
