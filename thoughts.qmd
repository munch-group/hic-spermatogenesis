...
#### Compartments 500kb resolition looks smoother

I made the same plot as above, but using a 500kb bin size in stead. It looks more smooth, but it is not obvious if we a removing valid information or noise. 
...
As they do not look that for all cell types, I wanted to see how the running average affected the E1 values:

{{< embed notebooks/03_compartments.ipynb#e1-fullmerged-all-500kb-running-avg >}}

Next I'm doing is to look at filtering of the matrix before balancing, to see if we have unreliable bins that influence the prediction. 